...
Email **security@archora.io**. Don't open public issues for security
problems. Include repro steps, affected version, and any PoC or logs.
...
- Local-first by default. The desktop app makes no network requests
  during a scan. License validation is the only planned outbound
  product request and is enumerated in the desktop CSP.
